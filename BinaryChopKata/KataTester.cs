...
{
    /* KARATE CHOP KATA  (http://codekata.com/kata/kata02-karate-chop/)
     * A binary chop (sometimes called the more prosaic binary search) finds the position of value in 
     * a sorted array of values. It achieves some efficiency by halving the number of items under 
     * consideration each time it probes the values: in the first pass it determines whether the 
     * required value is in the top or the bottom half of the list of values. In the second pass in 
     * considers only this half, again dividing it in to two. It stops when it finds the value it is 
     * looking for, or when it runs out of array to search. Binary searches are a favorite of CS lecturers.
     * 
     ...
     * different technique. Do the same the next day, until you have five totally unique 
     * implementations of a binary chop. (For example, one solution might be the traditional iterative 
     ...
     *     see if the frequency of these errors decreases (that is, do you learn from experience in one technique 
     ...
     * 3.  It’s fairly hard to come up with five unique approaches to a binary chop. How did you go about coming 
     *     up with approaches four and five? What techniques did you use to fire those “off the wall” neurons?
     *
     * Specifications
     * Write a binary chop method that takes an integer search target and a sorted array of integers. 
     * It should return the integer index of the target in the array, or -1 if the target is not in 
     * the array. The signature will logically be:
     * 
     *         int Chop(int, array_of_int)  
     *         
    ...
    public class KataTester
    {
        ...
        {
            testChopper(new ArrayBinarySearch());
        }

        [TestMethod]
        public void List_IndexOf()
        {
            testChopper(new ListIndexOf());
        }

        [TestMethod]
        public void RecursiveApproach()
        {
            testChopper(new RecursiveApproach());
        }

        [TestMethod]
        public void IterativeApproach()
        {
            testChopper(new IterativeApproach());
        }
    ...
    {
        int Chop(int target, int[] listOfNumbers);
    }
}
